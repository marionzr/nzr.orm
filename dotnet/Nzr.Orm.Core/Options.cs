using Microsoft.Extensions.Logging;
using System.Data;

namespace Nzr.Orm.Core
{
    /// <summary>
    /// DAO options
    /// </summary>
    public class Options
    {
        /// <summary>
        /// The default table schema to be used by the DAO.
        /// Default: dbo
        /// </summary>
        public string Schema { get; set; }

        /// <summary>
        /// If true, when no ColumnAttribute is defined for a property named Id, then the column name will be set as id_table.
        /// Default: true
        /// </summary>
        public bool UseComposedId { get; set; }

        /// <summary>
        /// If true, when no column name is provided for ForeignKeyAttribute, then the column name will be set as id_foreign_table.
        /// Default: true
        /// </summary>
        public bool InferComposedIdInForeignKeys { get; set; }

        /// <summary>
        /// If true, when no KeyAttribute is specified, the Dao will assume the Key is auto-generated.
        /// Otherwise will use the value given by the KeyAttribute that is true by default.
        /// Default: true
        /// </summary>
        public bool AutoGeneratedKey { get; set; }

        /// <summary>
        /// The naming style to be used by the DAO.
        /// Default: NamingStyle.LowerCaseUnderlined
        /// </summary>
        public NamingStyle NamingStyle { get; set; }

        /// <summary>
        /// The connection strings used to create connections.
        /// </summary>
        public string ConnectionStrings { get; set; }

        /// <summary>
        /// The isolation level used in the Transactions.
        /// Default: IsolationLevel.ReadCommitted
        /// </summary>
        public IsolationLevel IsolationLevel { get; internal set; }

        /// <summary>
        /// If true, will invoke the Trim method on string values returned in the DataReader.
        /// Default: true.
        /// </summary>
        public bool AutoTrimStrings { get; set; }

        /// <summary>
        /// The ILogger instance used to register log messages;
        /// </summary>
        public ILogger Logger { get; set; }

        /// <summary>
        /// If true, in the case the WHERE statements use the IN or NOT_IN operators and the args are empty (e.g. new double[] {})
        /// the SQL generated will be changed from 'WHERE column IN ()' to 'WHERE 'column IN ()' = 'empty args' and
        /// from 'WHERE column NOT IN ()' to 'WHERE 'column NOT IN ()' != 'empty args'.
        ///
        /// If false, will throw and OrmException.
        ///
        /// The default is true.
        /// </summary>
        public bool HandleEmptyInArgs { get; set; }

        /// <summary>
        /// Constructor.
        /// </summary>
        public Options()
        {
            Schema = "dbo";
            NamingStyle = NamingStyle.LowerCaseUnderlined;
            UseComposedId = true;
            InferComposedIdInForeignKeys = true;
            AutoGeneratedKey = true;
            IsolationLevel = IsolationLevel.ReadCommitted;
            AutoTrimStrings = true;
            HandleEmptyInArgs = true;
        }

        #region Builders

        /// <summary>
        /// Sets the Schema and return this instance as a builder set style.
        /// </summary>
        /// <param name="schema">The default table schema to be used by the DAO.</param>
        /// <returns>The Options instance.</returns>
        public Options WithSchema(string schema)
        {
            Schema = schema;
            return this;
        }

        #endregion
    }
}
